...
const INTERVAL = 6000;
const TRANSITION = 1800; // ms crossfade
...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative h-screen min-h-[700px] overflow-hidden flex items-end">
      {/* Background slides — all stacked, only active one is opaque */}
      ...
      {/* Slide dots */}
      <div className="absolute top-1/2 right-7 -translate-y-1/2 z-10 flex flex-col gap-2">
        {SLIDES.map((_, i) => (
          <button
            key={i}
            onClick={() => setCurrent(i)}
            className={`w-1 rounded-full transition-all duration-500 cursor-none ${
      ...
      <div
        ref={contentRef}
        className="relative z-10 w-full px-12 pb-18 grid grid-cols-2 items-end gap-10"
        style={{ opacity: 0, transform: "translateY(20px)" }}
      >
        <div>
          <div className="inline-block text-[10px] font-semibold tracking-[.22em] uppercase text-cream-dim border border-cream/25 px-3.5 py-1.5 rounded-sm mb-7">
            <T es="Fotografía · Video · Tours 3D · Dron · Planos" en="Photography · Video · 3D Tours · Drone · Floor Plans" />
          </div>
          <h1 className="text-[clamp(52px,7vw,96px)] font-light leading-none text-cream tracking-tight">
            <T
        ...
        <div className="flex flex-col items-end gap-8">
          <Image src="/logo.png" alt="a8 Real Estate Media Haus" width={200} height={70} className="h-14 w-auto drop-shadow-2xl opacity-90" />
          <p className="text-[13px] font-light leading-relaxed text-cream/65 max-w-[280px] text-right">
            <T
